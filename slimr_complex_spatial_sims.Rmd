---
title: "Complex Spatial Simulations with slimr"
...
spat_dat <- slim_extract_full(spat_run$output_data,
                                    "coordinates")
...
spat_script_more <- slim_script_render(spat_script, template = list(N = 500))
spat_script_more

spat_run_more <- slim_run(spat_script_more)

spat_dat_more <- slim_extract_full(spat_run_more$output_data,
                                         "coordinates")
...
coords <- slim_extract_full(final_gen, "coordinates")
...
```{r extract_genlight}
spat_gl <- slim_extract_genlight(final_gen)
plot(spat_gl)
```
...
dat_join <- coords %>%
  left_join(tibble(unique_ind_id = gsub("generation_1000", "", names(gen_cut)),
...
```{r run_again}
new_spat_script_more <- slim_script_render(new_spat_script,
                                            template = list(N = 500))
...
```{r gen_diff_again}
new_spat_gl <- slim_extract_genlight(new_spat_run$output_data %>%
...
dat_join <- dat_all %>%
  ...
  left_join(tibble(unique_ind_id = gsub("generation_1000", "", names(new_gen_cut)),
